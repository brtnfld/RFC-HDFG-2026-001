...
\texttt{cd\_values} at configuration time, and the validated string is also
stored verbatim in a new version~3 of the filter-pipeline message, so tools
can read a filter's configuration without loading its plugin. Filters opt in
via an updated plugin class (\texttt{H5Z\_class3\_t}) with two optional
callbacks. Existing call sites compile and function without modification, and
pipelines configured without parameter strings are still written in the old
format.
...
On-disk format
  & New pipeline-message version~3, written only when a parameter string
    is stored.  Without one, output is byte-identical to equivalent
    \texttt{H5Pset\_filter} output \\
Files from \texttt{H5Pappend\_filter}
  & Readable by all existing readers unless a parameter string was stored;
    \texttt{H5Pset\_libver\_bounds} forces the old format when needed \\
ABI
...
Version~3 is defined as an extensible record: each filter entry ends in a list
of typed, length-delimited extension blocks, and a reader skips blocks it does
not recognise unless they are marked critical.  The stored parameter string is
the first such block.  Follow-on work --- notably the large-configuration blob
channel of RFC-HDFG-2026-003 --- therefore adds block types rather than further
message versions, and remains readable by libraries that ship only this RFC.
