...
\texttt{H5Pget\_filter2}.  At filter-add time the library packs
\texttt{canonical\_name} into \texttt{cd\_values} so the name persists
in the file and tools can display it even when the plugin is not
installed.

\noindent\textbf{Compatibility note:} Because the packed
\texttt{canonical\_name} adds trailing slots to \texttt{cd\_values},
any v2 plugin that validates \texttt{cd\_nelmts} with an exact equality
check will fail on files written by a v3-registered plugin.  The
required fix is to change \texttt{cd\_nelmts == expected} to
\texttt{cd\_nelmts >= expected}.  This guidance is already in the HDF5
developer documentation; the full RFC includes a migration guide.
