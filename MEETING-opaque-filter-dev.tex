...
\midrule
1 & RFC-HDFG-2026-* (this document) & Design in progress \\
...
RFC-HDFG-2026-001 (shipping) introduced \texttt{H5Pappend\_filter} with a
...
\begin{minted}{c}
herr_t H5Pappend_filter_<name>(hid_t        dcpl,
...
\noindent\textit{Note: the final function name is under discussion.
``Opaque'' is avoided because \texttt{H5T\_OPAQUE} already carries that
term in HDF5 with a distinct meaning (tagged arbitrary bytes as a
datatype).}
...
The performance impact is a one-time read at \texttt{H5Dopen}; the
chunk I/O path is unchanged.
...
H5Pappend_filter_<name>(dcpl, SZ4_FILTER_ID, H5Z_FLAG_MANDATORY,
                        jit_buf, jit_size);
...
%%------------------------------------------------------------------
\section*{Dataset Lifecycle}

\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Event} & \textbf{What happens} \\
\midrule
\texttt{H5Dcreate} & Library writes bytes to the file \\
\texttt{H5Dopen}   & Library reads bytes back; filter restores internal state \\
\texttt{H5Dread}/\texttt{H5Dwrite} & Unchanged --- no impact on the I/O path \\
\texttt{H5Dclose}  & Library frees the in-memory copy \\
\bottomrule
\end{tabular}
\end{center}

%%------------------------------------------------------------------
\section*{What the Library Handles}

Filter authors do not need to consider:

\begin{itemize}[noitemsep]
  \item Writing and reading bytes to/from the file
  \item Parallel HDF5
  \item \texttt{h5repack} copying bytes to a new file
  \item \texttt{H5Pencode}/\texttt{H5Pdecode} portability
  \item Backward compatibility with older HDF5 files
\end{itemize}
...
\texttt{H5Pappend\_filter\_opaque} solves it directly; the bytes travel with
...
Because \texttt{H5Z\_class3\_t} is a new class, this does not affect
existing \texttt{H5Z\_class2\_t} filters.
...
\midrule
String config API --- \texttt{H5Pappend\_filter} (RFC-HDFG-2026-001)
  & Done \\
DXPL passed to filter callback (Issue~2, RFC-HDFG-2026-001)
  & In progress \\
Chunk coordinates in filter callback (Issue~3, RFC-HDFG-2026-001)
  & In progress \\
Large binary config API, name TBD (Issue~1, RFC-HDFG-2026-*)
  & Proposed; design in progress \\
RFC-HDFG-2026-*
...
\medskip
Existing filter registrations will require \textbf{no changes} when
RFC-HDFG-2026-* lands.
