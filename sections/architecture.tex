...
This subsection argues, use case by use case, that the TOML VFD/VOL profile
is sufficient to express all VFD stack configuration requirements.  The
reference point for these use cases is the custom LISP-style configuration
grammar proposed in the Lifeboat VFD configuration RFC~\cite{lifeboat-vfd-cl},
which motivates several of the design choices described here.
...
\textbf{How TOML handles it.}  Standard TOML key\,=\,value pairs cover this
...
\textbf{How TOML handles it.}  TOML has a native boolean type:
\texttt{true}/\texttt{false} (lowercase, unquoted).  This is a strict
improvement: the intent is self-documenting, the parser enforces the type, and
no integer-to-boolean coercion is required.  Example:
...
The Lifeboat integer workaround is unnecessary under TOML.
...
\textbf{How TOML handles it.}  TOML has no native binary type, but a quoted
...
The Lifeboat \texttt{--} prefix served only to distinguish binary data from
other terminal types within a grammar that lacks a string type capable of
carrying arbitrary content.  TOML's quoted strings remove the need for any such
disambiguation prefix.  The decoding convention (e.g., hex, base64) is
specified by the VFD and validated in its configuration entry point, exactly
as with all other parameter types.
...
\textbf{How TOML handles it.}  TOML inline tables and dotted keys are
...
\textbf{How TOML handles it.}  The same self-describing property is achieved
...
determine which VFD to load and configure, mirroring the Lifeboat approach
exactly, but using standard TOML key-value syntax rather than a custom grammar.
...
\textbf{How TOML handles it.}  An empty inline table \texttt{sec2 = \{\}} is
...
\textbf{How TOML handles it.}  The configuration group maps directly onto a
...
matching the Lifeboat group semantics exactly.  The writer/reader coordination
...
\textbf{Why breadth-first lexing is not needed with TOML.}  Lifeboat's
grammar does have named identifiers at every level, so the schema-isolation
goal could in principle have been met by a depth-first parser that simply
ignored unrecognised keys.  The breadth-first lexer trick is specifically a
consequence of Lifeboat's parser design choice to avoid building a recursive
in-memory parse tree and to defer parsing of nested sub-expressions to the
consuming VFD.  TOML parsers (including the vendored tomlc17 subset) build a
full recursive AST in a single pass, after which each VFD can locate the
keys it recognises by name and extract the \texttt{underlying\_vfd} sub-table
without scanning past unknown structure.  Schema isolation is preserved ---
each VFD reads only its own keys --- but it is achieved through named-key
lookup over a complete AST rather than through a context-sensitive lexer.
