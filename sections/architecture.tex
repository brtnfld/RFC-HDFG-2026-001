...
\item \texttt{description}---A human-readable, free-form summary of
  ...
  scientific data"}).  Unlike \texttt{canonical\_name}, the
  description is purely informational: it is \emph{not} persisted to
  disk, \emph{not} used for any identity comparison, and \emph{not}
  required to remain stable across plugin versions.  Plugin authors
  may set it to \texttt{NULL} if no useful description is available,
  in which case introspection tools fall back to displaying only the
  canonical name.  The field has no length limit beyond ordinary
  string conventions.  This is the v3 successor to the v1/v2 informal
  practice of writing a longer-form sentence into the \texttt{name}
  field; v3 separates the identifier (canonical\_name) from the
  prose (description) so that each can serve its purpose cleanly.
\end{itemize}
