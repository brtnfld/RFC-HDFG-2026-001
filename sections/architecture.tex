...
typedef struct H5Z_class_info_t {
    ...
    char          params_string[H5Z_CONFIG_STRING_MAX + 1];
} H5Z_class_info_t;
...
the free-form \texttt{description}, presence flags for the v3
\texttt{set\_config} / \texttt{get\_config} callbacks, and a
\texttt{params\_string} containing the filter's factory-default parameter
string.  The factory-default string is obtained by calling the filter's
\texttt{get\_config} callback with zero \texttt{cd\_values}; all built-in
filters fall back gracefully to their compiled-in defaults (e.g.\
\texttt{"level = 6"} for deflate, \texttt{"coding = \textbackslash"nn\textbackslash", pixels\_per\_block = 16"} for
SZIP).  Filters that do not implement \texttt{get\_config} leave
\texttt{params\_string} as an empty string.

The function triggers a dynamic plugin load if needed (same policy as
\texttt{H5Zfilter\_avail}); unknown filter IDs return \texttt{H5E\_NOFILTER}.

The \texttt{name} and \texttt{description} fields point into library-owned
storage; their lifetime extends until the filter is unregistered and they
must not be freed.  The \texttt{params\_string} field is a value-copy
embedded directly in the struct and requires no special handling.

This is the supported way to read a filter's \texttt{description} and
default \texttt{params\_string}, since neither field is persisted to disk
and both only exist when the plugin is loaded.  The original
