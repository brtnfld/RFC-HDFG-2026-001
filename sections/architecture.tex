...
\item \textbf{Reserved in-file binary config fields} (\texttt{write\_opaque},
  \texttt{read\_opaque}, \texttt{close\_opaque}) --- set to \texttt{NULL};
...
typedef struct H5Z_class3_t {
    ...
    H5Z_write_opaque_func_t     write_opaque;
    H5Z_read_opaque_func_t      read_opaque;
    H5Z_close_opaque_func_t     close_opaque;
} H5Z_class3_t;
\end{minted}
...
typedef struct H5Z_class_info_t {
    ...
    hbool_t       has_opaque_callbacks; /* reserved; true iff write_opaque != NULL */
} H5Z_class_info_t;
...
\subsection{Reserved Fields: In-File Opaque Configuration Data}
\label{sec:opaque-reserved}

\begin{outofscope}
Full activation of \texttt{H5Z\_PARAMS\_OPAQUE} and the three opaque callbacks
...
The long-term solution is to store such data as \emph{opaque} auxiliary data
inside the HDF5 file alongside the dataset's pipeline message.
...
\begin{itemize}
  \item \texttt{write\_opaque}---Called at dataset-create time to write the
    filter's opaque configuration data to the file and return a file address.
    If \texttt{NULL}, the library uses a default heap-object writer.
  \item \texttt{read\_opaque}---Called at dataset-open time to read the opaque
  ...
  \item \texttt{close\_opaque}---Called at dataset-close time to release the
    in-memory copy of the opaque data.
...
static const H5Z_class3_t my_filter = {
    ...
    NULL, NULL, NULL,    /* write_opaque, read_opaque, close_opaque (reserved) */
};
\end{minted}

\noindent The enum value \texttt{H5Z\_PARAMS\_OPAQUE~=~2} is likewise reserved
...
typedef enum {
    ...
    H5Z_PARAMS_OPAQUE   = 2,  /* reserved: in-file opaque config data;
