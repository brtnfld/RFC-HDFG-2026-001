...
#define H5Z_CLASS3_T_VERS     2   /* Version constant for H5Z_class3_t (H5Zdevelop.h) */
...
compatibility with code that already references it.  \texttt{H5Z\_CLASS3\_T\_VERS}
is defined in \texttt{H5Zdevelop.h} as a convenience constant for plugin authors
using \texttt{H5Z\_class3\_t}; it equals the literal value \texttt{2}.
...
new struct generations are added.
...
typedef herr_t (*H5Z_write_blob_func_t)(hid_t file_id, const void *buf,
                                          size_t size, haddr_t *addr_out);
typedef herr_t (*H5Z_read_blob_func_t) (hid_t file_id, haddr_t addr,
...
typedef struct H5Z_class3_t {
    ...
    /* Reserved: set to NULL — activated by RFC-HDFG-2026-002 */
    H5Z_write_blob_func_t     write_blob;
    H5Z_read_blob_func_t      read_blob;
...
typedef struct H5Z_class_info_t {
    ...
    hbool_t       has_blob_callbacks; /* reserved; true iff write_blob != NULL */
} H5Z_class_info_t;
...
Three NULL-initialized callbacks are reserved in \texttt{H5Z\_class3\_t} now,
before the struct ships, so that no new class version (\texttt{H5Z\_class4\_t})
will be required when the feature is activated:

\begin{itemize}
  \item \texttt{write\_blob}---Called at dataset-create time to write the
    filter's blob configuration data to the file and return a file address.
    If \texttt{NULL}, the library uses a default heap-object writer.
  \item \texttt{read\_blob}---Called at dataset-open time to read the blob
    data back from the file address stored in the pipeline message.
    If \texttt{NULL}, the library uses a default heap-object reader.
  \item \texttt{close\_blob}---Called at dataset-close time to release the
    in-memory copy of the blob data.
    If \texttt{NULL}, the library calls \texttt{free()}.
\end{itemize}
...
The library exports typed pack/unpack helpers from \texttt{H5Zdevelop.h} as
part of the developer API (not the stable public API):
\texttt{H5Zcd\_pack\_double}/\texttt{H5Zcd\_unpack\_double},
\texttt{H5Zcd\_pack\_float}/\texttt{H5Zcd\_unpack\_float},
\texttt{H5Zcd\_pack\_int64}/\texttt{H5Zcd\_unpack\_int64}, and
\texttt{H5Zcd\_pack\_string}/\texttt{H5Zcd\_unpack\_string}.
Plugin authors may use these helpers or implement the convention directly
using \texttt{memcpy} and the byte layout above.
