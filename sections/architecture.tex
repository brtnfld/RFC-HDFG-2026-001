...
\item \textbf{Quoted values:} A value may be enclosed in either double quotes
  ...
  \begin{quote}\ttfamily
    export HDF5\_FILTER\_PARAMS="desc='fast compression',level=6"
  \end{quote}
\item \textbf{Order of operations for parsing:} split the parameter string on commas
  ...
  value $\rightarrow$ apply quote processing to the value if it begins with
  \texttt{"} (double-quoted) or \texttt{'} (single-quoted).
...
\item \texttt{flags}---On input, contains the flags value from the API call. The
  ...
  \texttt{H5Pset\_filter\_params} writes into the DCPL. This gives the filter the
...
/* C++ does not support C99 compound literals.
   ...
   C++ callers MUST use the named-variable form instead (see below). */
#define H5Z_PARAMS_RAW(n, vals) \
    (H5Z_params_t{ H5Z_PARAMS_CDVALUES, { { (n), (vals) } } })
/* H5Z_PARAMS_STR expands to a declaration (static_assert), so
   &H5Z_PARAMS_STR(...) is syntactically invalid -- the compiler error
   may be a syntax error rather than a clean assertion message.
   If the resulting diagnostics are too noisy, implementors may
   substitute the undefined-function form below, which produces a
   clear linker/compiler error that includes the argument:
     extern H5Z_params_t
   ...
   Both forms meet the safety goal: the macro is unusable in C++. */
#define H5Z_PARAMS_STR(s) \
    static_assert(false, \
...
to \texttt{H5Pset\_filter\_params} is therefore valid C99: the literal's
...
\texttt{H5Pset\_filter\_params} accepts either a raw \texttt{cd\_values} array
...
\texttt{H5Pset\_filter\_params} returns \texttt{H5E\_NOFILTER} immediately
...
\texttt{H5Pset\_filter\_params} resolves the string into \texttt{cd\_values}, only
...
\texttt{H5Pset\_filter\_params} (\SectionRef{sec:append-filter}) is the primary API.
