...
\item \textbf{Quoted values:} If a value begins with a double quote (\texttt{"}) after
  ...
  represented as two consecutive double quotes (\texttt{""}). An unbalanced quote
...
\item The maximum parameter string length is 4096 bytes. The library defines
  the public constant \texttt{H5Z\_CONFIG\_STRING\_MAX} (= 4096). Strings exceeding
  this limit MUST be rejected with \texttt{H5E\_BADVALUE} by the library before
  the string is passed to any filter callback. Filters that legitimately
  require longer configuration (e.g., embedded dictionary paths) must use an
  alternative mechanism such as a file-path indirection key whose value is a
  short path string.
\item The maximum number of key-value parameters in a single parameter string
  is 64. The library defines the public constant \texttt{H5Z\_CONFIG\_MAX\_PARAMS}
  (= 64). Strings containing more than 64 comma-separated tokens MUST be
  rejected with \texttt{H5E\_BADVALUE}. This bound limits memory allocation in the
  duplicate-key detection logic and provides a defense against maliciously
  crafted strings.
...
The dispatch logic in \texttt{H5Zregister()} currently uses a two-way branch: if
\texttt{version == H5Z\_CLASS\_T\_VERS} (1), treat as \texttt{H5Z\_class2\_t}; otherwise, treat
as legacy \texttt{H5Z\_class1\_t} (since the first field of a v1 struct is the filter
ID, which is always $\geq 256$).

This MUST be extended to a three-way branch:

\begin{verbatim}
    if (version == 1) -> H5Z_class2_t
    else if (version == 2) -> H5Z_class3_t
    else if (version >= 256) -> legacy H5Z_class1_t
...
\texttt{version != H5Z\_CLASS\_T\_VERS} produces a hard error. The three-way branch
must account for this conditional compilation: under
\texttt{H5\_NO\_DEPRECATED\_SYMBOLS}, the dispatch accepts version~1 (v2 struct) and
version~2 (v3 struct), and rejects all other values.

The existing constant \texttt{H5Z\_CLASS\_T\_VERS} (currently 1) MUST NOT be changed.
If it were updated to~2, existing v2 plugin source code that initializes
\texttt{.version = H5Z\_CLASS\_T\_VERS} would silently produce a struct the library
interprets as v3, despite missing the \texttt{set\_config}/\texttt{get\_config}
fields---causing undefined behavior (reading uninitialized memory for callback
pointers). Instead, a new constant MUST be introduced:
...
typedef ssize_t (*H5Z_get_config_func_t)(
    unsigned          flags,
    size_t            cd_nelmts,
...
\item \texttt{flags}---On input, contains the flags value from the API call. The
  callback MAY modify this value. If the callback modifies flags, the modified
  value is what \texttt{H5Pset\_filter\_by\_name} writes into the DCPL. This gives the
  filter the final say on properties like \texttt{H5Z\_FLAG\_MANDATORY} vs.\
  \texttt{H5Z\_FLAG\_OPTIONAL}. If the callback modifies \texttt{flags}, it MUST push a
  warning onto the error stack via \texttt{H5Epush} with severity \texttt{H5E\_WARN} and
  class \texttt{H5E\_ARGS} before returning. This is not a hard error---callers that
  do not inspect the error stack are unaffected, and the modified flags value
  is still written to the DCPL. Callers that wish to detect flag overrides MAY
  check the error stack after \texttt{H5Pset\_filter\_by\_name} returns. Callbacks that
  accept the caller's flags without modification MUST NOT push this warning.
  This contract MUST be documented in \texttt{H5Zdevelop.h}.
...
\item \texttt{cd\_values\_size}---The capacity of the \texttt{cd\_values} array (0 when
  \texttt{cd\_values} is \texttt{NULL}).
...
\verysubsection{Unknown-key rejection}
A \texttt{set\_config} callback for a v3 filter MUST reject parameter strings
containing keys that the filter does not recognize, returning a negative value
and pushing \texttt{H5E\_BADVALUE} with a message identifying the unrecognized
...
silently ignored and produce unexpected default behavior. Because the string
API does not have the compile-time safety of named constants, strict
unknown-key rejection at \texttt{set\_config} time is essential for usability.
All built-in filter \texttt{set\_config} callbacks enforce this requirement.
This requirement MUST be documented in \texttt{H5Zdevelop.h} as part of the
...
\begin{itemize}
\item Reconstructs a human-readable parameter string from \texttt{cd\_values}.
\item Follows the standard HDF5 buffer-size query pattern: if \texttt{buf} is
  \texttt{NULL} or \texttt{buf\_size} is~0, returns the required size.
\item Returns the length of the string (excluding NUL) on success, negative on
  failure.
\item The reconstructed string is not required to be identical to the original
  input string. Semantic equivalence is sufficient (e.g., \texttt{"rate=3.5"} may
  be reconstructed as \texttt{"rate=3.500000"}). However, the output MUST conform
  to the same grammar as \texttt{set\_config} input (\SectionRef{sec:param-rules}),
  including support for bare keys and quoted values where appropriate, so that
  the output of \texttt{get\_config} is valid input to \texttt{set\_config} for
  round-tripping.
...
When a callback does modify \texttt{flags}, it MUST push a non-fatal warning
(\texttt{H5Epush} with severity \texttt{H5E\_WARN}, class \texttt{H5E\_ARGS}) so that callers who
want to detect the override can inspect the error stack without receiving a
hard error. Callers that do not check the stack are unaffected. See
...
\item \texttt{H5E\_CANTINIT}---The number of \texttt{cd\_values} elements returned by the
  filter's \texttt{set\_config} callback exceeds \texttt{H5Z\_COMMON\_CD\_VALUES} (8) and
  the target DCPL is associated with a compact dataset or shared object
  header, where the standard encoding allocates space for at most
  \texttt{H5Z\_COMMON\_CD\_VALUES} elements per filter entry. The library MUST
  return this error with an informative message rather than silently
  truncating the \texttt{cd\_values} array. Callers that encounter this error
  MUST either switch to a non-compact layout or use a filter whose
  \texttt{cd\_values} count does not exceed the limit. Note that
  \texttt{H5Pset\_filter} has the same implicit restriction; this API makes the
  limit explicit because user-friendly string configuration raises the
  expectation that all valid filter configurations are accepted.
\end{itemize}
...
Consequently, the reconstructed string may differ in formatting from the
original input (e.g., \texttt{"rate=3.5"} may become \texttt{"rate=3.500000"}). Consumers
MUST NOT rely on exact string equality between input and output.
...
optional parameter. If the key is present and \texttt{value\_buf} is non-\texttt{NULL},
the processed value is written to \texttt{value\_buf} (quotes stripped,
\texttt{""} unescaped---the filter sees the final usable string, not the raw grammar
tokens). If \texttt{val\_len} is non-\texttt{NULL}, it is set to the length of the
processed value string (excluding NUL). A bare key like \texttt{"fast\_mode"} (no
equals sign) returns positive with \texttt{val\_len = 0} and an empty
\texttt{value\_buf}, distinguishable from an absent key (returns \texttt{0}).
                     ...
                     size_t cd_values_size)
{
    ...
    *cd_nelmts = 1;
    ...
    /* ---- 4. Optionally override flags (push warning) ---- */
...
the same behavior as the existing integer-ID scan. On Windows, this
directory-wide enumeration is subject to time-of-check/time-of-use (TOCTOU)
races (a DLL may be added, removed, or replaced between the directory listing
and the \texttt{LoadLibrary} call). This is a pre-existing issue that affects
integer-ID plugin loading on Windows and is not introduced by this design.
However, implementations SHOULD document that name-based plugin discovery on
Windows carries the same TOCTOU caveats as the existing plugin loading
mechanism, and applications with security requirements SHOULD pre-register
all required plugins via \texttt{H5Zregister} rather than relying on on-demand
directory scanning.
