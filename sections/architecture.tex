...
\item \textbf{Inline table (nested):} A value may itself be a
  ...
  Nesting is intended for VOL connector and VFD configuration; filter
  \texttt{set\_config} callbacks are not expected to use it.
\end{itemize}
...
\midrule
\texttt{level = 6} & deflate (integer) \\
\texttt{mode = "rate", rate = 3.5} & ZFP (string + float) \\
\texttt{pixels\_per\_block = 16, coding = "entropy"} & SZIP \\
\texttt{scale\_type = "float\_dscale", scale\_factor = 4} & scaleoffset \\
\texttt{fast\_mode = true, level = 6} & boolean flag \\
\texttt{path = "/data/run\_1,v2/dict.bin"} & string value containing a comma \\
\texttt{stripe = \{size = 1048576, count = 4\}} & nested table (VOL/VFD) \\
\texttt{stripe.size = 1048576, stripe.count = 4} & dotted keys (equivalent to above) \\
\texttt{NULL} & shuffle, fletcher32 (no params) \\
\bottomrule
\end{tabularx}
\end{center}
