...
\item \texttt{canonical\_name}---The canonical string identifier for the
  ...
  to a pipeline, the library packs \texttt{canonical\_name} into the
  \texttt{cd\_values} array using \texttt{H5Zcd\_pack\_string()}, after
  \texttt{set\_config} has populated the filter's own parameters.  This
  packed representation persists in the file so that tools such as
  \texttt{h5dump} and callers of \texttt{H5Pget\_filter2} can display the
  filter's name even when the plugin is not installed.  The library
  additionally stores \texttt{canonical\_name} in the filter-pipeline
  message's name field, so \texttt{H5Pget\_filter2} can return it without
  having to unpack \texttt{cd\_values}.  When the plugin \emph{is}
  registered, \texttt{H5Pget\_filter2} returns \texttt{canonical\_name}
  directly from the runtime registry; without the plugin, it falls back
  to the stored pipeline name; with neither available, it falls back to
  the numeric filter ID formatted as a decimal string.

  \textbf{Length limit:} \texttt{canonical\_name} must not exceed 255
  bytes (excluding the NUL terminator).  \texttt{H5Zregister()} rejects a
  v3 registration where \texttt{strlen(canonical\_name) > 255} with
  \texttt{H5E\_BADVALUE}.  This bound guarantees that the packed
  \texttt{canonical\_name} consumes at most
  $\lceil 255/4 \rceil + 1 = 65$ \texttt{cd\_values} slots, leaving the
  remaining capacity exclusively for the filter's own parameters.

  \textbf{Legacy plugin hazard.}  Because \texttt{canonical\_name} is
  appended to \texttt{cd\_values}, a file written by a v3-aware library
  and then opened by an older library (e.g., HDF5~1.14.x) with a v2
  plugin will present the older plugin's execution callback with a
  \texttt{cd\_nelmts} value larger than the plugin expects.  Plugins that
  strictly validate \texttt{cd\_nelmts} will return an error, breaking
  read-compatibility for those datasets.

  \textbf{Mitigation.}  Plugin authors are strongly encouraged \emph{not}
  to validate \texttt{cd\_nelmts} as an exact equality check; the HDF5
  ...
  upgraded to v3 explicitly tolerate extra trailing slots.  Plugin
  authors who need strict validation should check only that
  \texttt{cd\_nelmts} is \emph{at least} the expected count.  This
  guidance is documented in \texttt{H5Zdevelop.h} and in the plugin
  migration guide produced alongside this RFC.
...
\item \textbf{cd\_values encoding is for display, not lookup.}  The
  \texttt{canonical\_name} packed into \texttt{cd\_values} provides
  display when the plugin is unloaded, but it is not usable as a lookup
  mechanism because files written prior to this feature do not carry
  the encoding.
...
name-based filter branch, the loader scans the configured plugin
directories, and a v3 plugin whose \texttt{canonical\_name} matches the
requested string is registered.  Its numeric \texttt{id} comes back
from the plugin's own struct.  This approach handles plugin name
changes without library updates, works for plugins never previously
loaded, and does not depend on file-format encoding.  v2 plugins (which
have no \texttt{canonical\_name}) do not participate in name-based
lookup.

The contract for callers that perform name-based lookup for an
optional filter must match the existing \texttt{H5Z\_FLAG\_OPTIONAL}
semantics for pipeline-apply: a ``not found'' result is a soft failure
that the pipeline skips, not a hard error.  The public API will expose
a flag analogous to \texttt{H5Z\_FLAG\_OPTIONAL} so callers can opt
into soft-failure semantics.
...
\texttt{canonical\_name} packed at the end of \texttt{cd\_values}.  Such
files remain readable by older HDF5 libraries through \emph{existing} v2
plugins, provided those plugins tolerate a \texttt{cd\_nelmts} value
larger than their baseline expectation (see the legacy plugin hazard
note in \SectionRef{sec:class3-struct}).  Plugin authors who perform
exact \texttt{cd\_nelmts} validation must update their callbacks to
accept trailing slots before their plugin is used with a v3-aware
library.

For v3 plugins, after \texttt{set\_config} has populated the filter's
\texttt{cd\_values}, the library appends the packed
\texttt{canonical\_name} string at the tail.  The packed bytes are never
seen by \texttt{set\_config} or \texttt{get\_config}.  When
\texttt{H5Pget\_filter2} is called, it returns \texttt{canonical\_name}
from the runtime registry if the plugin is loaded, falls back to the
name stored in the pipeline message at filter-add time, and finally
falls back to the numeric filter ID formatted as a decimal string.
...
string.  Because \texttt{canonical\_name} is packed into
\texttt{cd\_values} at filter-add time, the name is available even when
