...
\item \textbf{In-file blob configuration callbacks} (\texttt{write\_blob},
  \texttt{read\_blob}, \texttt{close\_blob}) --- a filter can store
  configuration data too large for \texttt{cd\_values} or the parameter
  string (\SectionRef{sec:blob}) as a binary blob persisted alongside the
  dataset's pipeline message.
\end{enumerate}
 ...
 * and read_blob.  See H5Z_write_blob_func_t / H5Z_read_blob_func_t. */
...
typedef struct H5Z_class3_t {
    ...
    H5Z_write_blob_func_t       write_blob;  /* NULL-able; see In-File Blob section */
...
typedef struct H5Z_class_info_t {
    ...
    bool          has_set_config;
    bool          has_get_config;
    bool          has_blob_callbacks; /* true iff write_blob/read_blob/close_blob are non-NULL */
} H5Z_class_info_t;
...
\texttt{set\_config} / \texttt{get\_config} and blob (\SectionRef{sec:blob})
callbacks.  The function triggers
...
\texttt{H5O\_PLINE\_VERSION\_4}, since \texttt{H5O\_PLINE\_VERSION\_3} is
defined by \SectionRef{sec:blob} for blob storage; see \SectionRef{sec:pline-v3})
capable of storing name-only pipeline entries that carry a canonical name
...
%% ======================================================================
%%  Section – In-File Blob Configuration Storage
%% ======================================================================
\section{In-File Blob Configuration Storage}
\label{sec:blob}

\subsection{Motivation}
\label{sec:blob-motivation}
...
65535-element \texttt{cd\_values} limit.  Two such cases were raised by
@robertu94 during the community review of this RFC
(HDFGroup/hdf5\_plugins\#255; see \texttt{LIBPRESSIO-FEEDBACK.md} in this
repository):

\begin{itemize}
\item \textbf{Use Case A --- JIT-compiled filter source (primary).}  SZ4's
  just-in-time compiler stores pre-processed source code for fast
  user-defined modules alongside the dataset.  There is no external
  artifact to reference; the bytes must travel with the dataset and be
  recovered on open.  This can reach multiple megabytes---a hard size
  limitation that neither \texttt{cd\_values} nor the parameter string can
  address.
\item \textbf{Use Case B --- reference to another dataset (secondary).}
  ROIBIN-SZ stores a spatial mask indicating which values use lossless
  compression.  The mask is a user-visible HDF5 dataset; only a reference
  to it (a small, fixed-size byte sequence) needs to travel with the
  filter's configuration.
\end{itemize}

Both cases are solved by the same mechanism: a binary blob, opaque to the
library, stored alongside the dataset's pipeline message and handed back to
the filter verbatim at open time.  This section specifies that mechanism as
a single, complete feature: the callback signatures, the on-disk format,
the public creation API, and the parallel I/O, \texttt{H5Pencode}/\texttt{H5Pdecode},
and \texttt{h5repack} semantics needed to use it correctly. \texttt{H5Z\_class3\_t}
has not yet shipped, so these callbacks are added as real, fully-specified
fields (\SectionRef{sec:class3-struct}) rather than reserved placeholders:
an earlier draft of this RFC reserved three \texttt{void~*} fields for later
activation by a follow-on RFC, but that approach was abandoned because
reserving an opaque placeholder does not avoid the version bump the
feature needs once real callback signatures are decided --- it only defers
the decision.  Deciding the signatures now, before \texttt{H5Z\_class3\_t}
ships, avoids that bump entirely.

\subsection{Blob Callbacks}
\label{sec:blob-callbacks}

The \texttt{write\_blob}, \texttt{read\_blob}, and \texttt{close\_blob}
fields added to \texttt{H5Z\_class3\_t} (\SectionRef{sec:class3-struct})
are all NULL-able.  When NULL, the library uses a default implementation
built on \texttt{H5HG} (the existing global-heap mechanism already used for
large variable-length attribute data): \texttt{H5HG\_insert} for writing,
\texttt{H5HG\_read} for reading, and \texttt{free()} for closing.  Plugins
implement these callbacks only when they need custom on-disk layout ---
for example, alignment requirements for GPU DMA transfers directly out of
the file buffer.  \texttt{H5Z\_blob\_loc\_t} (\SectionRef{sec:class3-struct})
is an opaque locator handed unchanged from \texttt{write\_blob} to the
pipeline-message encoder and later from the decoder to \texttt{read\_blob};
plugin authors do not construct or interpret its fields.
...
via the existing \texttt{set\_local} callback, or encodes them as a short
header inside the blob itself; the blob content is entirely filter-defined
bytes, so this requires no new library mechanism.

\subsection{New Public API: \texttt{H5Pappend\_filter\_blob}}
...
\verysubsection{Description}
Appends filter \texttt{id} to the pipeline on \texttt{plist\_id}, exactly as
\texttt{H5Pappend\_filter} does, and attaches \texttt{buf}/\texttt{size} as
the filter's blob.  The bytes are opaque to the library; they are handed to
the filter's \texttt{write\_blob} callback (or the default \texttt{H5HG}
writer) at \texttt{H5Dcreate} time and recovered via \texttt{read\_blob}
(or the default reader) at \texttt{H5Dopen} time.  \texttt{set\_config} is
not invoked for this entry point --- the blob path bypasses the
parameter-string layer entirely, since the whole point of the blob channel
is to carry data that does not fit through it.

\textbf{Decision: single entry point, no \texttt{H5Z\_params\_t} variant.}
An earlier draft of this RFC considered adding \texttt{H5Z\_PARAMS\_BLOB}
...
does not do so: \texttt{H5Z\_params\_t} is designed around values that fit
comfortably in a compound literal (\SectionRef{sec:append-filter}), and
forcing a multi-megabyte buffer through that union would either
double-store the buffer or complicate its lifetime rules for no benefit.
\texttt{H5Pappend\_filter\_blob} is a dedicated function with a signature
suited to its purpose; \texttt{H5Z\_params\_type\_t} is unchanged by this
section.
...
\item \texttt{id}---The filter identifier.  The registered filter must
  implement \texttt{write\_blob}/\texttt{read\_blob} or accept the library's
  default \texttt{H5HG}-based implementation.
...
\item \texttt{buf}---Pointer to the blob bytes.  The library copies these
  bytes into DCPL-owned storage before returning (see
  \SectionRef{sec:blob-encode-decode}); the caller's buffer may be freed or
  reused immediately after the call.
\item \texttt{size}---Length of \texttt{buf} in bytes.  There is no
  fixed upper bound analogous to \texttt{H5Z\_CONFIG\_STRING\_MAX}; the
...
\verysubsection{Errors}
\begin{itemize}
\item \texttt{H5E\_NOFILTER}---The filter plugin could not be found or loaded.
\item \texttt{H5E\_BADVALUE}---\texttt{buf} is \texttt{NULL} with nonzero
  \texttt{size}, or \texttt{size} is zero with non-NULL \texttt{buf}.
\end{itemize}

\subsection{Internal State: \texttt{H5Z\_filter\_info\_t} Additions}
...
\subsection{On-Disk Format: \texttt{H5O\_PLINE\_VERSION\_3}}
\label{sec:blob-format}

The existing \texttt{H5O\_PLINE\_VERSION\_2} per-filter layout is:

\begin{center}
\texttt{[id:2][flags:2][cd\_nelmts:2][cd\_values:cd\_nelmts×4][name:variable]}
\end{center}

\noindent Version~3 appends a \texttt{has\_aux} bit after the name and,
when set, the fixed-size \texttt{H5Z\_blob\_loc\_t} encoding (address plus
...
inline.  \texttt{H5O\_pline\_ver\_bounds[]} maps \texttt{H5F\_libver\_t} to
the pipeline message version; a DCPL with a blob-bearing filter requires a
libver upper bound that admits \texttt{H5O\_PLINE\_VERSION\_3}, exactly as
any other version-gated format feature does.

\textbf{Version-number note.}  This RFC introduces \texttt{H5O\_PLINE\_VERSION\_3}
for blob storage.  Two \emph{other} on-disk extensions mentioned elsewhere
in this document as future work --- name-only pipeline entries for
soft-failing optional name-based filters (\SectionRef{sec:append-filter})
and typed per-slot \texttt{cd\_values} storage (\SectionRef{sec:pline-v3})
--- were provisionally described in earlier drafts as also landing in
version~3.  Since version~3 is now defined by this RFC for blob storage,
those two still-deferred extensions would land in
\texttt{H5O\_PLINE\_VERSION\_4} or later whenever they are proposed.

Runtime I/O is unaffected: \texttt{H5Z\_pipeline}, the hot path for chunk
I/O, touches none of this.  The blob is configuration data, loaded once at
open time; compression/decompression performance is unchanged.

\subsection{Dataset Create / Open / Close Lifecycle}
\label{sec:blob-lifecycle}

\textbf{Create} (\texttt{H5D\_\_create}), after \texttt{H5Z\_set\_local} runs,
for each filter with \texttt{aux\_data~!=~NULL}:
\begin{enumerate}
\item Call \texttt{filter->cls->write\_blob(file\_id, aux\_data, aux\_size,
  \&aux\_loc)}, or the default \texttt{H5HG} writer if \texttt{write\_blob}
  is \texttt{NULL}.
\item Store \texttt{aux\_loc} in \texttt{H5Z\_filter\_info\_t}.
\item Encode the pipeline message at version~3 with \texttt{has\_aux} set
  and \texttt{aux\_loc} populated.
\end{enumerate}

\textbf{Open} (\texttt{H5D\_\_open}):
\begin{enumerate}
\item Decode the version-3 pipeline message; for each filter with
  \texttt{has\_aux}, read \texttt{aux\_loc}.
\item Call \texttt{filter->cls->read\_blob(file\_id, aux\_loc, \&aux\_data,
  \&aux\_size)}, or the default \texttt{H5HG} reader if \texttt{read\_blob}
  is \texttt{NULL}.
...
coordination that object-header message appends go through.  If every rank
in a collective \texttt{H5D\_\_create} called \texttt{H5HG\_insert}
...
\noindent\textbf{Decision:} \texttt{H5Pencode} serializes the blob's bytes
inline; \texttt{H5Pdecode} leaves the on-disk locator undefined.

\texttt{H5P\_\_ocrt\_pipeline\_enc} today encodes filter ID, flags, name,
and \texttt{cd\_values} --- all in-memory data; no file address appears in
...
derived, file-specific value; \texttt{aux\_data} is the canonical in-memory
representation.  \texttt{H5P\_\_ocrt\_pipeline\_enc} is therefore extended
to append, after each filter's \texttt{cd\_values}:
...
triggers \texttt{write\_blob} and assigns a real locator in whichever file
it is used with.  Serializing the locator instead of the bytes was
rejected: it would make an encoded DCPL valid only in its originating
file, breaking every \texttt{H5Pdecode}-then-\texttt{H5Dcreate} workflow in
a different file.  Returning an error for blob-bearing DCPLs was also
rejected as unnecessarily restrictive, since \texttt{aux\_data} is always
available in memory after \texttt{H5D\_\_open} or after
...
acceptable since \texttt{H5Pencode} is not a hot path and callers invoking
it on such a DCPL already know they hold unusual data.

\subsection{\texttt{h5repack} and \texttt{H5Pcopy} Behavior}
\label{sec:blob-repack}

\noindent\textbf{Decision:} No \texttt{h5repack}-specific code is needed;
\texttt{H5Pcopy} is extended to deep-copy \texttt{aux\_data}.
...
\emph{provided} \texttt{H5Pcopy} performs a deep copy of \texttt{aux\_data}:

\begin{enumerate}
\item \texttt{H5Dopen} on the source populates \texttt{aux\_data} in the
  source DCPL's filter entry (via \texttt{read\_blob} or the default reader).
\item \texttt{H5Pcopy(dcpl\_in)} must \texttt{malloc} + copy the
  \texttt{aux\_data} buffer for each filter that has one, leaving
  \texttt{aux\_loc} undefined in the copy (the destination address is not
  yet known).
...
\noindent\textbf{Decision:} Blob bytes are stored as a plain \texttt{H5HG}
global-heap object, not a new HDF5 object message type.
...
introduced in a later file-format version without breaking files written
under this design --- \texttt{H5O\_PLINE\_VERSION\_3}'s \texttt{aux\_loc}
...
Use Case B (\SectionRef{sec:blob-motivation}) stores an HDF5 reference to
a user-visible mask dataset as the filter's blob --- a small, fixed-size
byte sequence rather than a multi-megabyte payload, but otherwise an
ordinary use of \texttt{H5Pappend\_filter\_blob}.  Dereferencing that
reference to fetch the mask's actual contents requires a file handle,
which an earlier draft of this RFC proposed solving with a new
\texttt{H5Z\_get\_file\_id()} accessor or an additional
\texttt{open\_dataset} callback.

\noindent\textbf{Decision:} Neither is needed.  \texttt{read\_blob}
(\SectionRef{sec:blob-callbacks}) already receives \texttt{file\_id} ---
required for Use Case A's default \texttt{H5HG}-based storage --- and that
same handle is sufficient for Use Case B.  A filter that stores a
...
allocates and tracks itself) for use during subsequent \texttt{H5Z\_func2\_t}
...
\subsection{Under Discussion: Retrieving a Default-Stored Blob}
...
use cases in \SectionRef{sec:blob-motivation} therefore require custom
...
\texttt{H5Pget\_filter\_params\_by\_idx}.  This would let applications
and tools (e.g., an \texttt{h5dump~-p} annotation that a blob is present
and its size) retrieve default-stored blobs, and --- notably --- would let
a \emph{filter} consume its blob without implementing any storage
callbacks: the \texttt{set\_local} callback already receives the
dataset's \texttt{dcpl\_id} and could fetch and cache the blob there.
That combination (default storage + \texttt{set\_local} retrieval) would
cover Use Case A with no filter-side storage code at all.

Alternatives considered but not currently favoured: keeping the status quo
(filters that consume blobs must implement both storage callbacks), or
passing the blob into the filter callback through a new context parameter
(rejected earlier in this RFC's history for widening the hot-path
signature).  No decision is recorded here; this subsection exists to
solicit review feedback before the accessor is added to the
implementation.

\subsection{Boundaries of This Design}

The in-file blob feature specified above (\SectionRef{sec:blob}) is fully
in scope for this RFC: the callbacks, on-disk format, public API, and
parallel/encode/repack semantics are all part of what this RFC delivers.
Three narrow items are deliberately not addressed by this design, and are
not required for either Use Case A or Use Case B:

\begin{itemize}
\item Changes to \texttt{H5Z\_func\_t}, the runtime I/O callback used by
  \texttt{H5Z\_class1\_t}/\texttt{H5Z\_class2\_t} filters --- the hot path
...
The string-configuration feature (\SectionRef{sec:public-api}) introduces
\textbf{no on-disk format change}: files written via
\texttt{H5Pappend\_filter} are byte-identical to files written via
\texttt{H5Pset\_filter}, since \texttt{cd\_values} are stored in the existing
\texttt{H5O\_PLINE\_VERSION\_2} object-header message and the parameter
string is consumed at API call time rather than stored on disk.  This RFC
does introduce one on-disk format change overall: \texttt{H5O\_PLINE\_VERSION\_3}
for blob-configuration storage (\SectionRef{sec:blob-format}), used only by
datasets with a filter configured via \texttt{H5Pappend\_filter\_blob}.
Datasets that do not use the blob feature continue to be written at
version~2, byte-identical to \texttt{H5Pset\_filter} output.

\subsection{Limitations of the \texttt{cd\_values}-only Approach}

For filters that use string configuration but not the blob feature,
...
extension---provisionally \texttt{H5O\_PLINE\_VERSION\_4}, since
\texttt{H5O\_PLINE\_VERSION\_3} is defined by this RFC for blob storage
(\SectionRef{sec:blob-format})---that appends per-slot type tags alongside
\texttt{cd\_values}.  The \texttt{cd\_values} packing convention defined in
this RFC (\SectionRef{sec:cd-packing-convention}) is the normative
foundation: filters that adopt it today will be forward-compatible with
that extension without changes to their callbacks.
