...
\begin{itemize}
  ...
  \item guarantees exact IEEE\,754 double round-trips: C99 \texttt{DBL\_DECIMAL\_DIG}
    ...
    \texttt{strtod}; and
  \item is locale-independent: the exponent form uses only ASCII digits and
    \texttt{e}/\texttt{E}, unaffected by \texttt{LC\_NUMERIC}.
\end{itemize}
...
\item \textbf{Locale and precision requirement:} \texttt{get\_config}
  ...
  This format is strictly TOML\,v1.0 compliant (always contains a decimal
  point and exponent), guarantees exact IEEE\,754 \texttt{double} round-trips
  via 17 significant digits (\texttt{DBL\_DECIMAL\_DIG}), and is
  locale-independent.  Do not use \texttt{\%a} (hex-float, not valid TOML),
  \texttt{\%g} (may omit decimal point, producing a TOML integer), \texttt{\%f},
  or fewer than 17 significant digits.
