...
There is deliberately no \texttt{H5Z\_CLASS3\_T\_VERS} (or similarly-named)
convenience constant: a separate macro per struct generation looks like an
independent version counter rather than a shared \texttt{version} field, and
its value would need to be renumbered if it were ever reused as a default.
...
new struct generations are added. The library's own internal dispatch in
\texttt{H5Z.c} uses a package-private constant (not exposed in
\texttt{H5Zdevelop.h}) pinned to \texttt{2}, so that constant's meaning can't
drift if \texttt{H5Z\_CLASS\_T\_VERS\_MAX} is later bumped for a future struct
generation.
