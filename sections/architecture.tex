...
(\SectionRef{sec:pline-v3}) does \emph{not} add such entries --- every
entry still requires a numeric \texttt{id} --- so this remains future
work for a subsequent message revision.
...
\item \texttt{H5Pmodify\_filter} \emph{clears} the stored string for the
  ...
  for that entry reverts to \texttt{get\_config} fallback.
...
version-2 fields followed by an optional configuration-string block:
...
[new in v3]
  UINT16  config_length      (byte length of the string; 0 = no string)
  char    config[config_length]   (UTF-8, not NUL-terminated)
\end{Verbatim}

\texttt{config\_length} is bounded by \texttt{H5Z\_CONFIG\_STRING\_MAX}
(4096, \SectionRef{sec:param-overview}), which fits comfortably in the
16-bit field.  Configuration data too large for an inline string is the
domain of the follow-on blob RFC (\SectionRef{sec:blob-future}).  All
other message fields, including the name-slot rules for built-in versus
third-party filters (\SectionRef{sec:class3-struct}), are unchanged from
version~2.
...
\begin{enumerate}
\item If \emph{no} entry in the pipeline has a stored configuration
  string, the library writes version~2 (or version~1, per the existing
  rules) --- byte-identical to the output of an equivalent
  \texttt{H5Pset\_filter} program, at any libver bound.
\item If at least one entry has a stored string \emph{and} the file's
  libver upper bound admits the release that ships this feature
  (provisionally \texttt{H5F\_LIBVER\_V200}; the constant tracks the
  actual shipping release), the library writes version~3.
\item If a stored string is present but the libver upper bound is below
  that release, the library writes version~2 and omits the strings.  The
...
\texttt{H5Pappend\_filter\_blob} API, and an on-disk format extension to
carry the blob alongside the dataset's pipeline message.  Because
