...
\begin{minted}{c}
#ifdef __cplusplus
/* C++: brace-init is valid for aggregates */
#define H5Z_PARAMS_RAW(n, vals) \
    (H5Z_params_t{ H5Z_PARAMS_CDVALUES, { { (n), (vals) } } })
#define H5Z_PARAMS_STR(s) \
    (H5Z_params_t{ H5Z_PARAMS_STRING,   { { 0,   nullptr } } })
/* NOTE: H5Z_PARAMS_STR cannot use a C++ compound literal to carry 's'
   portably across all C++ versions; C++ callers should use the named-
   variable form shown in the lifetime note below. */
#else
...
\noindent C++ callers should prefer the named-variable form for
\texttt{H5Z\_PARAMS\_STRING} to avoid the C++ restriction on compound
literals carrying a pointer to a runtime expression:
