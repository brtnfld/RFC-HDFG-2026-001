...
\item \textbf{The pipeline-message name field is for display, not lookup.}
  The \texttt{canonical\_name} written into the pipeline message at
  filter-add time provides display when the plugin is unloaded, but it
  is not usable as a lookup mechanism because files written prior to
  this feature carry a v2 plugin's debug-comment name in that slot
  rather than a canonical identifier.
