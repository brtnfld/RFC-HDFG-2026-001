...
\textbf{Note: VFD/VOL configuration is out of scope for this RFC.}  The
constraints below are forward-looking design notes included for completeness.
They will be normative in a follow-on RFC that addresses VFD and VOL connector
...
\textbf{Note: This subsection is out of scope for this RFC.}  It is included
as forward-looking design context to demonstrate that the TOML grammar chosen
...
VFD/VOL profile constants) will be specified in a follow-on RFC.
...
\textbf{Note: Out of scope for this RFC.}  See \SectionRef{sec:vfd-use-cases}.
...
\textbf{Implementation status.}  \texttt{H5FDconfig\_get\_subtable\_str} and
the associated \texttt{H5FDconfig\_load\_from\_file} helper are proposed by
this RFC but are not yet present in the HDF5 source tree.  They are part of
the VFD/VOL configuration work that follows from this RFC's filter-focused
implementation.
