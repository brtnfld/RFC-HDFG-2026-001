...
\begin{itemize}
\item \texttt{name}---The canonical string identifier for the filter (e.g.,
  \texttt{"zfp"}, \texttt{"blosc2"}). This field occupies the same slot as
  \texttt{name} in \texttt{H5Z\_class2\_t} and carries the same semantics: it
  is the stable, unique string by which the filter identifies itself. It must
  not be \texttt{NULL}. This value is the designated lookup key for any future
  name-to-id mapping feature; plugin authors must treat it as an immutable
