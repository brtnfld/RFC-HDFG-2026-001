...
\item \texttt{canonical\_name}---The canonical string identifier for the
  ...
  Consequently, \texttt{canonical\_name} for built-ins is \emph{not}
  persisted to disk.  This is harmless: built-in filters are
  unconditionally present in every HDF5 build, so the runtime
  registry always supplies their \texttt{canonical\_name} to
  \texttt{H5Pget\_filter2}, and the ``plugin unloaded, read name
  from disk'' fallback path never engages for them.  The disk-slot
  mechanism is therefore the persistence channel for third-party
  filters; built-ins rely on the runtime registry only.
  ...
  (e.g., one read from disk carrying \texttt{canonical\_name}, the
