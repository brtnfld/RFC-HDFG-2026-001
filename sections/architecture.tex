...
introduces no build or runtime dependency and is consistent with the VFD/VOL
configuration string convention in HDF5~1.14.
...
A quoted empty string (\texttt{key=""}) is also rejected with
\texttt{H5E\_BADVALUE}.
...
field in \texttt{H5Zregister()} and \texttt{H5PL\_\_open} via \texttt{memcpy}
...
behavior of \texttt{H5Pset\_filter}. The pipeline is subject to the existing
...
\texttt{cd\_values} array. The two APIs are fully interchangeable on the same
...
\texttt{H5Pget\_filter2} returns the \texttt{description} field if it is non-\texttt{NULL},
and falls back to the \texttt{name} field otherwise. This preserves the existing
behavior of returning human-readable descriptive text.
\texttt{H5Pget\_filter\_params\_by\_idx} complements \texttt{H5Pget\_filter2} by returning
the parameter string rather than the filter description.
...
\texttt{char[]}       & $1{+}\lceil\text{len}/4\rceil$ &
                          slot[0]\,=\,byte length; remaining slots hold 4-byte
                          null-padded chunks & \texttt{H5Z\_SLOT\_STR\_LEN/DATA} \\
\bottomrule
\end{tabularx}
\end{center}
...
\item The file's \texttt{H5Pset\_libver\_bounds} upper bound is at least
  \texttt{H5F\_LIBVER\_V200} (the version constant introduced with this
  feature; the exact value is defined in \texttt{H5Fpublic.h} for the
  release that ships this RFC).
...
\texttt{H5F\_LIBVER\_V200}), the library emits a version-2 entry
...
\begin{minted}{c}
/* In H5Fpublic.h */
#define H5F_LIBVER_V200  5   /* HDF5 2.0 -- introduces H5O_PLINE_VERSION_3 */
\end{minted}
...
\texttt{H5F\_LIBVER\_V200}, the encoder falls back to version~2 and issues
...
\begin{minted}{c}
H5Pset_libver_bounds(fapl, H5F_LIBVER_V200, H5F_LIBVER_LATEST);
\end{minted}
