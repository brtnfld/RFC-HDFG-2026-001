...
p.u.str = b"level=6"
H5Pappend_filter(plist_id, H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, &p)
\end{minted}
...
! String form -- binds directly to H5Pappend_filter_str_c
SUBROUTINE h5pappend_filter_str_f( &
...
! Raw cd_values form -- binds directly to H5Pappend_filter_raw_c
SUBROUTINE h5pappend_filter_raw_f( &
...
The HDF5 C++ wrapper library (\texttt{H5Cpp}) adds two overloaded
\texttt{appendFilter} methods to \texttt{DSetCreatPropList}.  The existing
\texttt{setFilter} method (which wraps \texttt{H5Pset\_filter}) is unchanged.
C++ callers never interact with \texttt{H5Z\_params\_t} or
\texttt{H5Z\_PARAMS\_STR} directly; the overloads hide the union entirely.

\begin{minted}{cpp}
// In DSetCreatPropList.h
class DSetCreatPropList : public PropList {
public:
    // Existing -- unchanged, wraps H5Pset_filter
    void setFilter(H5Z_filter_t filter_id, unsigned int flags = 0,
                   size_t cd_nelmts = 0, const unsigned int *cd_values = nullptr);

    // New: string form -- resolves to H5Z_PARAMS_STRING
    void appendFilter(H5Z_filter_t filter_id, unsigned flags,
                      const std::string &params);

    // New: raw cd_values form -- resolves to H5Z_PARAMS_CDVALUES
    void appendFilter(H5Z_filter_t filter_id, unsigned flags,
                      size_t cd_nelmts, const unsigned cd_values[]);
};
\end{minted}

Each \texttt{appendFilter} overload constructs the appropriate
\texttt{H5Z\_params\_t} on the stack (using the named-variable form, since
\texttt{H5Z\_PARAMS\_STR} is disabled in C++) and calls
\texttt{H5Pappend\_filter}.  No new C entry point is required.  A matching
\texttt{getFilterParams} method wraps \texttt{H5Pget\_filter\_params\_by\_idx}:
...
dcpl.appendFilter(H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, "level=6");

// Raw cd_values form (compiler selects size_t + const unsigned[] overload)
unsigned cd[] = { 6 };
dcpl.appendFilter(H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, 1, cd);
