...
\item The \texttt{set\_config} callback (\texttt{H5Z\_\_szip\_set\_config} in
  ...
  \texttt{pixels\_per\_block} is an even integer in \texttt{[2,~32]}.  The
  callback does not delegate to \texttt{H5Pset\_szip}.
...
\item The \texttt{set\_config} callback (\texttt{H5Z\_\_scaleoffset\_set\_config}
  ...
  validates that \texttt{scale\_factor} is non-negative, and populates
  \texttt{cd\_values} directly.  The callback returns
  \texttt{H5E\_BADVALUE} if either key is absent.  The callback does not
  delegate to \texttt{H5Pset\_scaleoffset}.
...
not both correctly-rounded per IEEE~754.  Because the
\texttt{cd\_values} packing convention is layout-deterministic only
after the \texttt{double} value is in hand, a per-rank difference in
the string-to-double conversion step propagates into
...
\verysubsection{Blob-configured filters}
\texttt{h5repack} requires no code changes to correctly copy a dataset
whose filter carries a blob (\SectionRef{sec:blob}).  It already copies
DCPLs exclusively via \texttt{H5Pcopy}; once that function's pipeline-copy
callback is extended to deep-copy \texttt{aux\_data}
(\SectionRef{sec:blob-repack}), the existing
\texttt{H5Dopen}/\texttt{H5Pcopy}/\texttt{H5Dcreate} lifecycle moves the
blob's bytes to the destination file automatically, with a new on-disk
locator assigned by \texttt{write\_blob} at the destination
\texttt{H5Dcreate}.
...
The value is the literal output of \texttt{get\_config}, enclosed in single
...
the string returned by \texttt{get\_config} contains one, it is escaped as
\texttt{\textbackslash\textquotesingle} in the h5dump output. For filters
that do not implement \texttt{get\_config}, \texttt{H5Pget\_filter\_params\_by\_idx}
provides a fallback string that formats the raw \texttt{cd\_values} as a
colon-separated list (see \SectionRef{sec:introspection}), so
\texttt{PARAMS\_STRING} can be displayed for all filters regardless of
whether they implement \texttt{get\_config}.
...
%% ======================================================================
%%  Section – Blob Storage Implementation Notes
%% ======================================================================
\section{Blob Storage Implementation Notes}
\label{sec:blob-impl}

This section maps the design in \SectionRef{sec:blob} to the library source
files it touches.
...
\texttt{src/H5Opline.c} & \texttt{H5O\_PLINE\_VERSION\_3} encode/decode of
...
\texttt{tools/src/h5repack/} & No changes needed; see the
  \textit{Blob-configured filters} note in \SectionRef{sec:cli-tools}. \\
\texttt{test/} & New tests: create/open/close, parallel write/broadcast,
  ...
  cross-file copy for a blob-configured filter
  (\SectionRef{sec:test-blob}). \\
\bottomrule
\end{tabularx}
\end{center}
