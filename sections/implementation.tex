...
callers, it would call \texttt{H5Pset\_filter\_params} from Cython by
declaring \texttt{H5Z\_params\_t} as a Cython \texttt{cdef} struct and
populating it by field assignment (the same named-variable pattern used
for C++, since C99 compound literals are not available in Cython):
...
p.u.str = b"level=6"
H5Pset_filter_params(plist_id, H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, &p)
\end{minted}
...
helper \texttt{H5Pset\_filter\_params\_str\_c} expects a NUL-terminated
\texttt{const char~*}.  The caller is responsible for appending
\texttt{C\_NULL\_CHAR}, following the same convention used throughout the
existing HDF5 Fortran binding:

Critically, the C helpers that receive \texttt{CHARACTER(KIND=C\_CHAR)}
string arguments (\texttt{H5Pset\_filter\_params\_str\_c},
\texttt{H5Zconfig\_get\_param\_c}) must strip any trailing Fortran space
padding before passing the string to the underlying C functions.  The
implementation should scan backward from the NUL terminator and remove
trailing spaces, consistent with \texttt{H5\_verify\_signature} and
similar helpers already in the library.  The test suite must exercise
this with \texttt{CHARACTER} variables that are \emph{wider} than the
string literal assigned to them, e.g.:

\begin{minted}{fortran}
! Padded to 32 chars; C helper must strip trailing spaces before
! passing to H5Pset_filter_params
CHARACTER(LEN=32) :: param_str = "level=6"
CALL h5pset_filter_params_f(plist, H5Z_FILTER_DEFLATE_F, &
                             H5Z_FLAG_MANDATORY_F,        &
                             TRIM(param_str)//C_NULL_CHAR, hdferr)
\end{minted}

Callers should prefer \texttt{TRIM()} to remove padding before
concatenating \texttt{C\_NULL\_CHAR}, as shown above.  The C helper
performs a defensive strip as a second line of defence.
...
The HDF5 C++ wrapper library (\texttt{H5Cpp}) uses overloaded methods on
\texttt{DSetCreatPropList}, mirroring the approach taken for Fortran generics
and Java overloads.  C++ callers never interact with \texttt{H5Z\_params\_t}
or \texttt{H5Z\_PARAMS\_STR} directly; the overloads hide the union entirely.

Two overloaded \texttt{setFilterParams} methods are added to
\texttt{DSetCreatPropList}:

\begin{minted}{cpp}
// In DSetCreatPropList.h
class DSetCreatPropList : public PropList {
public:
    // String form -- resolves to H5Z_PARAMS_STRING
    void setFilterParams(H5Z_filter_t filter_id, unsigned flags,
                         const std::string &params);

    // Raw cd_values form -- resolves to H5Z_PARAMS_CDVALUES
    void setFilterParams(H5Z_filter_t filter_id, unsigned flags,
                         size_t cd_nelmts, const unsigned cd_values[]);
};
\end{minted}

Each overload constructs the appropriate \texttt{H5Z\_params\_t} on the
stack (using the named-variable form, since \texttt{H5Z\_PARAMS\_STR} is
disabled in C++) and calls \texttt{H5Pset\_filter\_params}.  No new C
entry point is required.  A matching overloaded \texttt{getFilterParams}
method wraps \texttt{H5Pget\_filter\_params\_by\_idx}:

\begin{minted}{cpp}
    // Returns the parameter string for the filter at filter_idx
    std::string getFilterParams(unsigned filter_idx) const;
\end{minted}

\verysubsection{C++ usage example}

\begin{minted}{cpp}
DSetCreatPropList dcpl;

// String form (compiler selects const std::string& overload)
dcpl.setFilterParams(H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, "level=6");

// Raw cd_values form (compiler selects size_t + const unsigned[] overload)
unsigned cd[] = { 6 };
dcpl.setFilterParams(H5Z_FILTER_DEFLATE, H5Z_FLAG_MANDATORY, 1, cd);

// Introspection
std::string p = dcpl.getFilterParams(0);  // e.g. "level=6"
\end{minted}
