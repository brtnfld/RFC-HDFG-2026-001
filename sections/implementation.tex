...
access to the registry---including lookups by \texttt{h5repack} and
mutations during plugin auto-registration---are serialized.
...
user-facing calls. Plugin auto-registration (\SectionRef{sec:plugin-auto-reg})
occurs within the same API call scope and is therefore covered by the same
lock. No new locking mechanisms are introduced.

\verysubsection{Future-proofing note}
Name registry lookups (used internally by \texttt{h5repack} to resolve a
canonical name to a filter ID before calling \texttt{H5Pset\_filter2}, and by
\texttt{h5dump} for filter display) are pure read operations with no I/O side
effects. If the
library moves toward finer-grained concurrency
...
present identical dataset creation property lists (DCPLs). Because filter
...
one rank and broadcast it to all others (e.g., via
\texttt{H5Pset\_libver\_bounds} followed by serialisation through
\texttt{H5Pencode}/\texttt{H5Pdecode}) rather than having each rank
independently invoke \texttt{H5Pset\_filter2}.

\verysubsection{Failure mode and plugin loading order}

\noindent\textbf{Decision:} \texttt{H5Pset\_filter2} loads the plugin
...
all plugin interaction, \texttt{H5Pset\_filter2} must run the filter's
...
plugin is unavailable, \texttt{H5Pset\_filter2} returns \texttt{H5E\_NOFILTER}
at the call site rather than deferring failure to \texttt{H5Dcreate} or
I/O time.  For Parallel HDF5, all ranks call \texttt{H5Pset\_filter2}
...
design. The old aliases continue to work unchanged. The new
\textit{filtername}\texttt{=}\textit{params} form requires \texttt{h5repack} to
resolve the canonical name to a filter ID (using the library-internal name
registry populated during \texttt{H5Zregister}) before calling \texttt{H5Pset\_filter2}.
...
\emph{forbidden} canonical filter name and is rejected by the name
registry.  The \texttt{h5repack} parser always treats \texttt{UD=} as the
legacy syntax and handles disambiguation itself before resolving the filter
name to an ID and calling \texttt{H5Pset\_filter2}---it is not the library's
responsibility to resolve this ambiguity.  Any filter that previously relied on the name
\texttt{UD} uses a different canonical name.
