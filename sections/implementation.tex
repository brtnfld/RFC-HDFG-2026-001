...
Fortran does not support C unions, so \texttt{H5Z\_params\_t} cannot be
expressed as a portable Fortran derived type.  Instead, the Fortran
binding exposes a generic interface \texttt{h5pset\_filter\_params\_f} with
two specific procedures, one for each parameter mode.  Fortran's
\texttt{INTERFACE} block resolves the correct specific at the call site
based on the type of the \texttt{params} argument.
...
! String form -- calls H5Pset_filter_params with H5Z_PARAMS_STRING
...
! Raw cd_values form -- calls H5Pset_filter_params with H5Z_PARAMS_CDVALUES
...
The JNI-side shim for each specific constructs the appropriate
\texttt{H5Z\_params\_t} value (using \texttt{H5Z\_PARAMS\_STR} or
\texttt{H5Z\_PARAMS\_RAW}) and calls \texttt{H5Pset\_filter\_params}.
No new C entry point is required.

The following additional subroutines are added to the Fortran binding:

\begin{minted}{fortran}
! Pipeline introspection -- wraps H5Pget_filter_params_by_idx
SUBROUTINE h5pget_filter_params_by_idx_f( &
    prp_id, filter_idx, params_buf, params_len, hdferr)
  INTEGER(HID_T),   INTENT(IN)    :: prp_id
  INTEGER,          INTENT(IN)    :: filter_idx
  CHARACTER(LEN=*), INTENT(OUT)   :: params_buf
  INTEGER(SIZE_T),  INTENT(OUT)   :: params_len
  INTEGER,          INTENT(OUT)   :: hdferr

! Parameter lookup helper -- wraps H5Zconfig_get_param
FUNCTION h5zconfig_get_param_f( &
  ...
  INTEGER                         :: found   ! htri_t: >0 found, 0 absent, <0 error
\end{minted}
...
Each JNI implementation constructs the appropriate \texttt{H5Z\_params\_t}
on the native side and calls \texttt{H5Pset\_filter\_params}.
