...
access to the registry---including lookups during \texttt{H5Pget\_filter\_name\_by\_idx}
and mutations during plugin auto-registration---are serialized.
...
Name registry lookups (used internally by \texttt{h5repack} for
name-to-ID resolution and by \texttt{h5dump} for filter display) are pure
read operations with no I/O side effects. If the
