...
Each built-in filter that accepts parameters implements \texttt{set\_config}
and \texttt{get\_config} so that the string API is fully functional
without third-party plugins.  The parameterless built-ins (shuffle,
fletcher32, nbit) implement neither callback, per
\SectionRef{sec:append-filter}: the library handles their
\texttt{NULL}/empty-string configuration directly, and a non-empty
string supplied to one of them is rejected with \texttt{H5E\_UNSUPPORTED}
without ever reaching a callback.
...
added to the existing \texttt{hdf.hdf5lib.H5} class:
