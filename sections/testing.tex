...
\begin{itemize}
\item \texttt{ts-01} (\textbf{concurrent name lookups}): Spawn $N$ threads
  ($N \geq 8$) that each call \texttt{H5Pget\_filter\_name\_by\_idx} for built-in filter
  pipeline entries in a tight loop.  No data race or crash occurs.
\item \texttt{ts-02} (\textbf{concurrent registration and name lookup}): One thread
  calls \texttt{H5Zregister} with a new v3 plugin while other threads concurrently
  call \texttt{H5Pget\_filter\_name\_by\_idx} on pipeline entries that may include that
  filter.  Lookups see a consistent state (name absent or name present);
