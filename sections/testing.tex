...
is documented in the field description (\SectionRef{sec:class3-struct}); a
plugin migration guide produced alongside this RFC will cover the
\texttt{>=} validation convention and other v2$\rightarrow$v3 migration
steps.  Plugins must check that \texttt{cd\_nelmts} is \emph{at least} the
expected count, not exactly equal.  The test suite includes a cross-version
read test (\texttt{reg-06}) to verify this behaviour.
...
\item \texttt{reg-05}: All existing \texttt{h5repack} invocations with the legacy filter
  ...
  \texttt{h5repack}.
\item \texttt{reg-06} (\textbf{cross-version read}): Write a dataset using a v3-registered
  plugin with a non-\texttt{NULL} \texttt{filter\_title} (so that extra \texttt{cd\_values}
  slots are appended).  Read the file using a build of the library that presents only
  a v2 version of the same plugin.  Verify that the dataset is read correctly---the
  v2 plugin's execution callback must accept the larger \texttt{cd\_nelmts} and
  return correct data.
