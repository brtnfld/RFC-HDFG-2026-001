...
\item \texttt{reg-02}: \texttt{H5Pset\_filter\_params} with \texttt{params=NULL} for a v2 filter
  succeeds (\texttt{NULL} is treated as ``no parameters'', equivalent to
  \texttt{H5Z\_PARAMS\_RAW(0, NULL)}).
\item \texttt{reg-03}: \texttt{H5Pset\_filter\_params} with \texttt{H5Z\_PARAMS\_STRING} for a
...
\item \texttt{par-01} (\textbf{DCPL consistency}): All ranks call
  \texttt{H5Pset\_filter\_params} with the same filter ID and parameter string; verify
...
\item \texttt{par-02} (\textbf{rank failure propagation}): Simulate a missing plugin
  ...
  Verify that the failing rank returns an error from \texttt{H5Pset\_filter\_params} (or from
...
\item \texttt{ts-03} (\textbf{concurrent \texttt{H5Pset\_filter\_params}}): Threads each create
  their own DCPL and call \texttt{H5Pset\_filter\_params} concurrently for the same built-in
...
The string API adds parsing cost at \texttt{H5Pset\_filter\_params} call time:
...
\texttt{H5Pset\_filter\_params} and reuse it across multiple \texttt{H5Dcreate}
