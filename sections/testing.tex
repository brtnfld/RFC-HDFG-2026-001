...
Upgrading a plugin from \texttt{H5Z\_class2\_t} to \texttt{H5Z\_class3\_t}
changes the role of the \texttt{name} field from a freeform debug comment
to a canonical identifier.  The new \texttt{filter\_title} field provides
a short human-readable label that is packed into \texttt{cd\_values} and
persists in the file: \texttt{H5Pget\_filter2} extracts \texttt{filter\_title}
from \texttt{cd\_values} when present, ensuring backward-compatible
output for applications that read the \texttt{name[]} output parameter
even when the plugin is not installed.
Plugin authors should populate \texttt{filter\_title} when upgrading to
maintain continuity for downstream consumers (see R31).
...
\verysubsection{After (v3): canonical identifier + filter title}
In \texttt{H5Z\_class3\_t}, the \texttt{name} field serves as the canonical identifier
used for registry lookup. It conforms to the naming rules in
...
\texttt{"bitshuffle"}). Descriptive prose is not appropriate and is not used.

The new \texttt{filter\_title} field in \texttt{H5Z\_class3\_t} provides a short
human-readable label (e.g., \texttt{"ZFP Compression"}). It must be printable
ASCII with no leading or trailing whitespace and no equals signs. It is not used
for registry lookup or any programmatic dispatch. When non-\texttt{NULL}, it is
packed into \texttt{cd\_values} as a reserved key and therefore persists in the
file, making it available to tools even when the plugin is not installed.
...
If \texttt{filter\_title} is absent, it falls back to the \texttt{name} field (the
canonical identifier). This means that plugin authors who populate
\texttt{filter\_title} during the v2-to-v3 upgrade preserve full backward
compatibility for applications that display the \texttt{H5Pget\_filter2} output,
and the label remains visible even without the plugin loaded.
Plugins that leave \texttt{filter\_title} as \texttt{NULL} will cause a
user-visible change in the \texttt{H5Pget\_filter2} output (the shorter canonical
name appearing where a longer label was previously shown). Applications
...
\begin{itemize}
\item \textbf{Registry lookup:} Use \texttt{H5Z\_class3\_t.name} as the canonical
  identifier for filter registration. The canonical name is stable and is used
  by \texttt{h5repack} for name-to-ID resolution.
\item \textbf{Human-readable display:} Provide a short label in
  ...
  \texttt{filter\_title = "ZFP Compression"}.
...
The string configuration is resolved at configuration time into
\texttt{cd\_values}.  When the file's \texttt{H5Pset\_libver\_bounds} upper bound
is below \texttt{H5F\_LIBVER\_V300}, the library writes a standard
\texttt{H5O\_PLINE\_VERSION\_2} entry that is byte-identical to
\texttt{H5Pset\_filter} output---no format change for such files.

When the upper bound is at least \texttt{H5F\_LIBVER\_V300} and the filter
uses typed slots (float, double, or string parameters packed via the
\texttt{H5Z\_cd\_pack\_*} helpers), the library writes
\texttt{H5O\_PLINE\_VERSION\_3}, which appends a per-slot type-tag block after
the \texttt{cd\_values} array.  Files containing a version-3 pipeline message
require a reader that recognises this version; earlier readers return an error
for such files (standard HDF5 object-header versioning behavior).

All-integer pipelines (all slots \texttt{H5Z\_SLOT\_UINT32}) always produce
version-2 output regardless of the libver bound.
...
The following process action is completed before this feature ships.

\verysubsection{Filter names in the HDF Group registry}
Canonical string names are added to the HDF Group's external filter
...
developed plugins may claim the same canonical name, and the
first-registered-wins collision policy provides no recourse after the fact.
\begin{itemize}
\item The HDF Group publishes canonical names for all filters currently
  listed in the integer-ID filter registry~\cite{hdf5registeredfilters}
  alongside their numeric IDs.
\item The registration process for new filter IDs is extended to
...
\item The published canonical name is the single source of truth. Plugin
  authors use the published name as their \texttt{H5Z\_class3\_t.name}
  field.
\end{itemize}
This is a process requirement, not a technical change to the library.
Implementation may proceed in parallel, but the feature does not ship
...
\item \texttt{reg-04b}: For a v3 plugin with a non-\texttt{NULL} \texttt{filter\_title}
  ...
  from \texttt{cd\_values}, not the canonical \texttt{name}. For example, a plugin
  with \texttt{name = "zfp"} and \texttt{filter\_title = "ZFP Compression"} returns
  \texttt{"ZFP Compression"}, even when the plugin is not loaded.
...
\item \texttt{reg-05}: All existing \texttt{h5repack} invocations with the legacy filter
  ...
  \texttt{h5repack}, provided the libver upper bound remains below
  \texttt{H5F\_LIBVER\_V300}.  With a V300 upper bound and typed slots, the
  output file contains \texttt{H5O\_PLINE\_VERSION\_3} entries and is not
  byte-for-byte identical to pre-feature output.
\end{itemize}
...
\item \texttt{par-01} (\textbf{DCPL consistency}): All ranks call
  \texttt{H5Pset\_filter2} with the same filter ID and parameter string; verify
...
\item \texttt{par-02} (\textbf{rank failure propagation}): Simulate a missing plugin
  ...
  Verify that the failing rank returns an error from \texttt{H5Pset\_filter2} (or from
...
\item \texttt{ts-03} (\textbf{concurrent \texttt{H5Pset\_filter2}}): Threads each create
  their own DCPL and call \texttt{H5Pset\_filter2} concurrently for the same built-in
...
\begin{itemize}
\item \texttt{nr-01}: A v3 plugin whose \texttt{name} field matches a name already in
  the registry is silently ignored (first-registered-wins); the filter itself
  is registered by ID, but no new name entry is created. Verify by attempting
  an \texttt{h5repack} name-lookup for the colliding name: it resolves to the
  first-registered filter ID, not the later one.
\item \texttt{nr-02}: After \texttt{H5Zunregister}, all name registry entries for the
  unregistered filter ID are removed. Verify with a memory checker that all
  internally copied name strings are freed (no leak).
\item \texttt{nr-03}: A v3 plugin whose \texttt{name} field violates the naming rules
