...
Each accessor returns \texttt{htri\_t} ($>0$ found, $0$ absent, $<0$ error)
and writes the decoded value through its output parameter
(\SectionRef{sec:config-get-param}); it does not return the value itself.
Unless noted otherwise, ``returns \emph{X}'' below is shorthand for ``the
call returns positive (found) and writes \emph{X}'' to the output
parameter --- e.g., \texttt{H5Zconfig\_get\_int} returns \texttt{6}'' means
\texttt{H5Zconfig\_get\_int(params, key, \&out)} returns a positive
\texttt{htri\_t} with \texttt{out~==~6}.  parse-04 and parse-09 are the
exception: there they report the \texttt{htri\_t} return value itself
(\texttt{0}, absent), since no output value exists in that case.
...
\texttt{parse-09} & \texttt{LEVEL = 6}
  & valid TOML; but \texttt{H5Zconfig\_get\_int(params, "level", \ldots)} returns
  \texttt{0} (absent) --- keys are case-sensitive, so \texttt{LEVEL} does not
  match \texttt{level} \\
...
\item \texttt{rt-E01}: Passing a non-NULL, non-empty \texttt{params} to a parameterless
  built-in (\texttt{shuffle}, \texttt{fletcher32}, \texttt{nbit}) returns
  \texttt{H5E\_UNSUPPORTED}: these filters implement no \texttt{set\_config}
  callback, so this is the ``filter found but does not implement
  \texttt{set\_config}'' case (\SectionRef{sec:append-filter}), not a
  rejected parameter string.
\item \texttt{rt-E01b}: Passing \texttt{NULL} or an empty parameter string
  ...
  appended.
\item \texttt{rt-E02}: Passing an unrecognized key (e.g., \texttt{levl = 6}
  instead of \texttt{level = 6} for deflate) to any built-in that implements
  \texttt{set\_config} returns \texttt{H5E\_BADVALUE} with an error message
  identifying the unrecognized key. v3 filters reject unknown keys
...
\item \texttt{par-02} (\textbf{rank failure propagation}): Simulate a missing plugin
  ...
  \texttt{H5Pappend\_filter} itself, per the eager-loading contract
  (\SectionRef{sec:append-filter}: a string-configured filter is resolved
  at \texttt{H5Pappend\_filter} time, never deferred to \texttt{H5Dcreate}),
  and that the application can detect this and abort collectively without
  an MPI deadlock.  A failure surfacing instead at \texttt{H5Dcreate} would
  itself be a bug: it would mean ranks disagree about which call is
  collective, which is exactly the divergence this test exists to catch.
