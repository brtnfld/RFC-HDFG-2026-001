...
messages, which persist each entry's parameter string verbatim alongside
...
set-then-get round trip \emph{does} hold --- this RFC's primary path has
no lossy round-trip.  The residual risk is confined to the fallback
...
\item \texttt{fmt-01} (\textbf{stored-string round-trip}): Create a dataset
  ...
  the input string byte-for-byte, with the plugin \emph{not} loaded
  (e.g., \texttt{HDF5\_PLUGIN\_PATH} unset).
...
\item \texttt{fmt-12} (\textbf{built-in canonical name block}): Configure
  deflate via \texttt{H5Pappend\_filter} with \texttt{level = 6}; verify
  the entry carries an \texttt{H5O\_PLINE\_EXT\_NAME} block containing
  \texttt{"deflate"}, and that \texttt{h5dump~-p} reports both the name
  and \texttt{PARAMS\_STRING} with no plugin path configured.  Configure
  a third-party filter the same way and verify it carries \emph{no} name
  block --- its name is in the version-2 slot and must not be duplicated
  (\SectionRef{sec:config-storage-rules}).  Configure deflate via raw
  \texttt{cd\_values} and verify the entry carries no blocks at all and
  the message is version~2.
\end{itemize}
