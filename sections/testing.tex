...
\item \texttt{fmt-07} (\textbf{\texttt{H5Pmodify\_filter} clears the
  string}): Append a filter via the string form, call
  \texttt{H5Pmodify\_filter} with altered \texttt{cd\_values}, and verify
  the stored string is cleared and introspection reverts to the
  \texttt{get\_config} fallback (\SectionRef{sec:dcpl-retention}).
