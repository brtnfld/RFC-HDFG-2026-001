...
\texttt{H5Z\_class3\_t} renames the \texttt{name} field present in
\texttt{H5Z\_class2\_t} to \texttt{canonical\_name} and makes it
required.  Filters are still addressed by integer ID for programmatic
dispatch (assigned by the HDF Group filter registry~\cite{hdf5registeredfilters}),
but \texttt{canonical\_name} now serves as both the stable string
identifier and the human-readable label returned by
\texttt{H5Pget\_filter2}.  The library packs \texttt{canonical\_name}
into \texttt{cd\_values} at filter-add time, so the name persists in
the file and is available to tools even when the plugin is not
installed.
...
\verysubsection{After (v3): \texttt{canonical\_name} packed into \texttt{cd\_values}}
\texttt{H5Z\_class3\_t} replaces \texttt{name} with the required
\texttt{canonical\_name} field --- a stable, unique string identifier
(e.g., \texttt{"zfp"}, \texttt{"blosc2"}) that doubles as the
human-readable display label.  The library packs it into
\texttt{cd\_values} at filter-add time so the name persists in the file.

\verysubsection{Impact on \texttt{H5Pget\_filter2}}
For v3 plugins, \texttt{H5Pget\_filter2} returns \texttt{canonical\_name}
from the runtime registry when the plugin is loaded.  When the plugin is
not loaded, it falls back to the name stored in the pipeline message at
filter-add time (sourced from the packed \texttt{canonical\_name} in
\texttt{cd\_values}).  If neither is available, it falls back to the
...
\item \textbf{Pick a stable canonical name.}  When upgrading to v3,
  set \texttt{canonical\_name} to a short, stable string identifier
  for the plugin class (e.g., \texttt{"zfp"}, \texttt{"blosc2"}).  Do
  not include version numbers, mode descriptors, or other variable
  ...
  become a v3 plugin with \texttt{canonical\_name = "zfp"}.
\item \textbf{No action required for consumers of \texttt{H5Pget\_filter2}
  ...
  function correctly; the displayed string is now the
  \texttt{canonical\_name}.
\end{itemize}
...
When a file written with a v3-registered filter (which appends the
packed \texttt{canonical\_name} to \texttt{cd\_values}) is opened by
an older HDF5 library through a v2 plugin, the plugin's execution
callback receives
...
\item \texttt{reg-04b}: For a v3 plugin, \texttt{H5Pget\_filter2}
  returns \texttt{canonical\_name} from the runtime registry when the
  plugin is loaded.  For example, a plugin with
  \texttt{canonical\_name = "zfp"} returns \texttt{"zfp"}.
\item \texttt{reg-04c}: For a v3 plugin that is not loaded but whose
  ...
  \texttt{canonical\_name} stored in the pipeline message at filter-add
  time (sourced from the packed \texttt{canonical\_name} in
  \texttt{cd\_values}).  If no stored name is available, it falls back
  to the numeric filter ID as a decimal string.
...
\item \texttt{reg-06} (\textbf{cross-version read}): Write a dataset
  using a v3-registered plugin (which appends the packed
  \texttt{canonical\_name} to \texttt{cd\_values}).  Read the file
  using a build of the library that presents only a v2 version of the
  same plugin.  Verify that the dataset is read correctly --- the v2
  plugin's execution callback must accept the larger \texttt{cd\_nelmts}
