...
For configuration data too large for \texttt{cd\_values} or the parameter
string --- e.g., pre-processed source for a JIT-compiled filter, or a
reference to a spatial mask stored as a separate dataset --- this RFC also
adds in-file blob configuration storage: three new \texttt{H5Z\_class3\_t}
callbacks (\texttt{write\_blob}/\texttt{read\_blob}/\texttt{close\_blob}),
the \texttt{H5Pappend\_filter\_blob} API, and a corresponding
\texttt{H5O\_PLINE\_VERSION\_3} on-disk format used only by datasets that
opt into it (see \S\ref{sec:blob}).
\end{abstract}
