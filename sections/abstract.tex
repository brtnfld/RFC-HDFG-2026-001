...
optional \texttt{set\_config}/\texttt{get\_config} callbacks and a required
\texttt{canonical\_name} field that serves as both the stable string
identifier and the display name; it is written into the filter-pipeline
message at filter-add time so it is available to tools even when the
